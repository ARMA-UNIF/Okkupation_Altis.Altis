...
if(_onLadder == 1) exitWith{player globalChat "Du kannst die Kiste nicht bewegen, wenn Du auf einer Leiter bist!";};
...
else
{
	...
	if(_tempVar) exitwith {
		hint format["Dieses Objekt gehört zu %1 und sie sind n der nähe, Du kannst das nicht nehmen.", _objet getVariable "R3F_Side"]; R3F_LOG_mutex_local_verrou = false;
	};
	_objet setVariable ["R3F_Side", (playerSide), true];
	...
	else
	{
		...
		else
		{
			...
			_action_menu_release_relative = player addAction [("<t color=""#06ef00"">" + STR_R3F_LOG_action_relacher_objet + "</t>"), "scripts\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\relacher.sqf", false, 5, true, true];
			_action_menu_release_horizontal = player addAction [("<t color=""#06ef00"">" + STR_RELEASE_HORIZONTAL + "</t>"), "scripts\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\relacher.sqf", true, 5, true, true];
			_action_menu_45 = player addAction [("<t color=""#06ef00"">Kiste um 45° drehen?</t>"), "scripts\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\rotate.sqf", 45, 5, true, true];
